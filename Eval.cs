...
namespace appBusinessFormBuilder
{
	...
	public class FunctionEventArgs : EventArgs
	{
        ...
        public List<string> ParametersString { get; set; }
        public double Result { get; set; }
        public string ResultString { get; set; }
            ...
            if (parser.Peek() != ')')
            {
                ...
                while (!parser.EndOfText)
                {
                    if (parser.Peek() == ',')
                    {
                        ...
                        if (parenCount == 1)
                        {
                            string sParameter = EvaluateParameterString(parser, paramStart);
